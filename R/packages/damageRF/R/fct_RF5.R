...
rf5 = function(
    ...
    identity, data, id, id_vars, target_vars, target, row_limit = NA, trees = 1000, grid = 11,
    n_boots = 30, seed = NA, tune_metric = "mae", train_ratio = 0.80, n = 100,
    randomize_target = FALSE, testrun = FALSE, minimize_memory = FALSE
  ...
  if ("target" %in% colnames(data)) {
    stop("Error in rf5: Found 'target' in colnames(data)")
  }

  # Exclude from modeling
  id_vars = intersect(colnames(data), id_vars)
  # Exclude from modeling (except "target" itself)
  target_vars = setdiff(intersect(colnames(data), target_vars), target)
  # Keep for modeling purpose
  pred_vars = setdiff(colnames(data), c(id_vars, target_vars))
