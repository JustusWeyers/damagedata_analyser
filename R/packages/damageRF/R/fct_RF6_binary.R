#' Title: Random Forest 6 - Classification
#'
...
#' @param min_sensitivity Optional: Minimum sensitivity for minority class threshold selection (default 0.95)
...
rf6_binary = function(
  ...
  quantiles = NA, min_sensitivity = 0.95
  ...
  if ("target" %in% colnames(data)) {
    stop("Error in rf6: Found 'target' in colnames(data)")
  }
  ...
  # Minderheitsklasse und zugehörige Pred-Spalte automatisch bestimmen
  minority_class = weights_df |>
    dplyr::slice_max(weight, n = 1, with_ties = FALSE) |>
    dplyr::pull(target)

  pred_col = paste0(".pred_", minority_class)

  # Optimaler Schwellenwert für Klasse 1 (Minderheitsklasse) auf OOF-Daten
  ...
  # Höchster Schwellenwert bei dem Sensitivity noch >= min_sensitivity
